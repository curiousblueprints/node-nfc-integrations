...
export default async function process_sonos_command(received_text) {
  const received_text_cleaned = received_text.replaceAll('â', '"').replace(/\s/g,'').replaceAll('“', '"').replaceAll('”', '"').replaceAll('', '');
  try {
    const request = JSON.parse(received_text_cleaned);
  } catch (e) {
    console.log(received_text_cleaned + " is not valid json.");
    for (let i = 0; i < received_text_cleaned.length; i++) {
      console.log(received_text_cleaned[i]);
      console.log(received_text_cleaned.charCodeAt(i));
    }
    console.log(e);
  }
