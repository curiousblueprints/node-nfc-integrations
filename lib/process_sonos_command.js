...
export default async function process_sonos_command(received_text) {
  ...
  if (request.action === 0) {
    ...
    console.log('Attempting API call command');
    ...
    instance.post(actionConfig.homeAssistantBaseUrl + '/api/services/input_boolean/toggle', JSON.stringify({
      'entity_id': 'input_boolean.nfc_toggle_'+ request.param
    }), {
      headers: {
        'Authorization': 'Bearer ' + action.config.homeAssistantAccessToken,
